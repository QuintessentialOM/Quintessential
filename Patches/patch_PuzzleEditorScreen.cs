...
class patch_PuzzleEditorScreen{
	...
	private void DisplayEditorPanelScreen(){
		...
		Vector2 corner = (class_115.field_1433 / 2 - size / 2 + new Vector2(-2f, -11f)).Rounded();
		Bounds2 panelSize = Bounds2.WithSize(corner + new Vector2(457f, 94f), new Vector2(980f, 712f));

		// draw headers
		class_140.method_317(class_134.method_253("Products", "FULL LENGTH"), corner + new Vector2(489f, 774f), 904, false, true);
		class_140.method_317(class_134.method_253("Reagents", ""), corner + new Vector2(489f, 506f), 904, false, true);
		class_140.method_317(class_134.method_253("Mechanisms and Glyphs", ""), corner + new Vector2(489f, 237f), 904, false, true);

		Puzzle myPuzzle = self.Get<Maybe<Puzzle>>("field_2789").method_1087();

		// draw inputs/outputs
		for(var row = 0; row < 2; row++){
			var puzzleIOs = row == 0 ? myPuzzle.field_2771 : myPuzzle.field_2770;
			for(var column = 0; column < 4; ++column){
				Bounds2 puzzleIoBox = Bounds2.WithSize(corner + new Vector2(495f, 576f) + new Vector2(column * 236, row == 0 ? -28f : -297f), new Vector2(226f, 226f));
				if(puzzleIOs.Length > column){
					class_135.method_272(self.Get<enum_138>("field_2788") == 0 ? class_238.field_1989.field_94.field_805 : class_238.field_1989.field_94.field_808, puzzleIoBox.Min);
					var isHover = false;
					if(self.Get<enum_138>("field_2788") == 0){
				...
				else if(self.Get<enum_138>("field_2788") == 0){
		...
		Vector2 rulesCorner = corner + new Vector2(494f, 184f);
